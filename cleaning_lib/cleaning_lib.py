...
def checkType(column, type):
	...
	for x in range(0, len(col)):
		...
		if letterCount >= numberCount:
			return column, numberRow
		else:
			return column, letterRow
...
#
# 	title = "We detected that this column, " + str(colName) + ", has mostly " + majority + ". We have found " + str(numberCount) + " numbers and " + str(letterCount) + " letters/strings. What type should this column be??"
